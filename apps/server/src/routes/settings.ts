...
async function readOverlayHostEnabled(db: Db): Promise<boolean> {
  const [row] = await db
    .select()
    .from(settings)
    .where(eq(settings.key, OVERLAY_HOST_ENABLED_KEY))
    .limit(1);
  return row?.value !== "0";
}
  ...
  .patch(
    ...
    },
    {
      body: t.Object({
        ...
        /** Enable/disable the desktop in-game overlay host (Wayland). */
        overlayEnabled: t.Optional(t.Boolean()),
