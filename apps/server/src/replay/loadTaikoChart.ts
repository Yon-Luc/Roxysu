...
import { parseTaikoChart, type TaikoHitObject } from "@roxysu/osu-chart";
import {
  getOsuDataPath,
...
export async function loadTaikoChartForScore(
  db: Db,
  score: {
  ...
  | { ok: true; chart: LoadedTaikoChart }
...
> {
  ...
  if (row.rulesetShortName !== "taiko") {
    return { ok: false, status: 422, error: "Beatmap is not taiko" };
  }
