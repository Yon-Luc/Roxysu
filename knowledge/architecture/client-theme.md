...
3. Beatmap preview, score rewatch, and embedded preview sit on a dark overlay over cover art. That chrome uses `on-media` / `on-media-muted` and does **not** follow `ink`. **Not:** `text-ink` on `bg-black/*` overlays.
...
`html.light` flips canvas/ink/status hues. `--color-on-media` is defined only in `@theme` so it stays near-white in both appearances.

Tailwind maps `--color-*` to utilities (`text-ink`, `text-danger`, `bg-warning/15`, `text-on-media`).

## Important symbols

- `apps/server/public/global.css` — token definitions
