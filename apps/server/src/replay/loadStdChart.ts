...
import { parseStdChart, type StdHitObject } from "@roxysu/osu-chart";
import {
  getOsuDataPath,
...
/**
 * Load Standard hit objects for a score, preferring the score's beatmapHash
 * when it still resolves in the local files store.
 */
export async function loadStdChartForScore(
  db: Db,
  score: {
...
> {
  ...
  if (row.rulesetShortName !== "osu") {
    return { ok: false, status: 422, error: "Beatmap is not standard" };
  }
